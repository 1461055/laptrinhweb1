<?php
	...
	if(isset($_SESSION["GioHang"]))
	{
		...
		for($i = 0; $i < $soLuongSanPham; $i++)
		{
			...
			// Lấy thông tin sản phẩm: Giá Sản phẩm, số lượng tồn.
			$sql = "SELECT GiaBan, SoLuongTon FROM sanpham WHERE MaSanPham = $id";
			$result = DataProvider::ExecuteQuery($sql);
			$row = mysqli_fetch_array($result);
			...
			// Thêm 1 dòng mới vào bảng ChiTietDonDatHang
			$sql = "INSERT INTO chitietdondathang(MaChiTietDDH, MaDonDatHang, MaSanPham, GiaBan, SoLuong) VALUES('$maChiTietDonDatHang', '$maDonDatHang', $id, $giaSanPham, $sl)";
			DataProvider::ExecuteQuery($sql);

			// Update lại số lượng tồn của bảng SanPham
